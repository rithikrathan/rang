...
from types import Point, CurvePoint, KolamCurvePattern, GridCell, KolamGrid, Line, Dot, KolamPattern
    ...
    def draw_kolam(cls, M):
        ...
        for i in range(m):
            for j in range(n):
                if flippedM[i][j] > 0:
                    dots.append(Dot(
                        id=f"dot-{i}-{j}",
                        center=Point(x=(j + 1) * cls.CELL_SPACING, y=(i + 1) * cls.CELL_SPACING),
                    ...
                    if 0 <= idx < len(KOLAM_CURVE_PATTERNS):
                        ...
                        curvePoints = [CurvePoint(
                            ...
                            controlX=(j + 1 + p.controlX) * cls.CELL_SPACING if p.controlX else None,
                            controlY=(i + 1 + p.controlY) * cls.CELL_SPACING if p.controlY else None
        ...
        grid = KolamGrid(
                ...
                cells=[[GridCell(
                    ...
                    dotCenter=Point(x=(j + 1) * cls.CELL_SPACING, y=(i + 1) * cls.CELL_SPACING)
        ...
        return KolamPattern(
                ...
                dots=dots,
                symmetryType="1D",
                dimensions={"width": (n + 1) * cls.CELL_SPACING, "height": (m + 1) * cls.CELL_SPACING},
